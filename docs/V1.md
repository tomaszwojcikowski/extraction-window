...
| Balance | `test:balance` **55–85%** WR; mixed lose channels (hp + storm min); stuck &lt;20% — **met:** **59.2% ±4.3** over 500 fresh seeds (Wave 8 gate now runs 300 generated seeds), hp+storm present, stuck 0% |
...
| `test:balance` / full `playtest` | PASS — **19/30 wins (63% WR)**; 61.5% ±6.7 over 200 fresh seeds (`scripts/probe-wr.ts`). **Superseded by Wave 8:** the 30-seed gate later read green at a true 52.6%; current band is **59.2% ±4.3** over 500 seeds |
| Lose mix | hp=7, storm=0, energy=2, stuck=0 (stuck **0%** of suite); hp owns 76% over 300 seeds — still 75% after Wave 8 and open |
