...
| Kit + pressure | Consumables, tool + armor equip, EM, quiet, hazards | `inventory.ts`, `emStress.ts` |
...
| Optional depth | 3 room-quest kinds, survey, NPCs | See **Room quests** below |
...
| Check | Result |
|-------|--------|
| `playtest:cohere` | PASS — 15 sectors, 22 enemies, 18 items, lore keys ok |
| `test:unit` | PASS — 13 files / 108 tests |
| `playtest:smoke` | PASS — 5/8 wins (63%), 0 crashes, illegal=0 |
| `test:balance` / full `playtest` | PASS — **19/30 wins (63% WR)**; 66.5% over 200 fresh seeds |
