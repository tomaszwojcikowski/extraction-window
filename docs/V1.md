...
9. **Wave 14 — Copy coherence** — strip Window-as-clock from player strings; align help/HUD/PADD with Power-only rules — **next** ([`ADOM_DEPTH.md`](ADOM_DEPTH.md)).
10. **Wave 15 — Presentation debt** — GameScene shrink; human feel gates from experiment passes — **planned**.
11. **Ship tag** — `v1.0.0` when copy + human feel gates pass — **pending**.

---

## Latest balance read (Waves 11–13)

| Check | Result |
|-------|--------|
| `test:unit` | PASS — 51 files / 394 tests |
...
**Open before tag:** lore/copy drift (`src/data/lore.ts` still teaches Window clock); human browser feel gate; stuck rate watch on held-out probe.

**Tag:** Not created. Headless oracle is green; **tag when Wave 14 copy + human feel pass**.

---

## Phase 6 — Playtest harden (historical)
