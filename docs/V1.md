...
**Status:** all seven kinds are live. Base pool is `salvage | purge | vent_seal | decode`; the three v1-deferred kinds were re-enabled by sector band in [`ADOM_DEPTH.md`](ADOM_DEPTH.md) Wave 3 (`pickRoomQuestKind`, [`src/sim/roomQuest.ts`](../src/sim/roomQuest.ts)).

| Kind | Availability | Why |
|------|--------------|-----|
| `salvage` | All sectors | Teaches kit/loot; low cognitive load |
| `purge` | All sectors | Teaches combat wake + light/aggro under pressure |
| `vent_seal` | All sectors (multi-site) | Teaches sealant + vent/EM; fits Meridian bus/hazard fantasy |
| `decode` | All sectors (Wave 1) | Hold-position ID fantasy pairs with tiered salvage |
| `relay_chain` | Sectors 4–10 (Wave 3) | Midgame multi-site with pattern-buffer fail-safe favor |
| `stabilize` | Sectors 5–6 (Wave 3) | Midgame hazard-pass favor |
| `calibrate` | Sectors 7–12 (Wave 3) | Late-mid timer multi-site with storm-shelter favor |

**Survey / NPCs:** keep survey as soft storm/XP refund; NPCs as lore/ally hail only (not required for extract).

**Guardrail unchanged:** optional quests never gate extract, and the pool only widens while `test:balance` holds the 55–85% WR band.
...
- Opportunistic comment cleanup in `sim/` (`EPS drip`, coolant-for-Type-9 notes)
...
| Quests | Active pick pool documented in **Room quest prune** and matching `pickRoomQuestKind`; no quest gates extract |
| Balance | `test:balance` **55–85%** WR; mixed lose channels (hp + storm min); stuck &lt;20% — **met:** **60% WR** (18/30), hp+storm present, stuck 0% |
...
3. **Systems prune** — Room-quest pick pool; scripted-event pass; pillar hints — **done** (this pass).
...
| Check | Result |
|-------|--------|
| `playtest:cohere` | PASS — 15 sectors, 22 enemies, 25 items, lore keys ok |
| `test:unit` | PASS — 13 files / 108 tests |
| `playtest:smoke` | PASS — 5/8 wins (63%), 0 crashes, illegal=0 |
| `test:balance` / full `playtest` | PASS — **18/30 wins (60% WR)** |
| Lose mix | hp=9, storm=3, energy=0, stuck=0 (stuck **0%** of suite) |
