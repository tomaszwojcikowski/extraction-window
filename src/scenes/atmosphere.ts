...
export function drawMenuPlate(
  g: G,
  x: number,
  y: number,
  w: number,
  h: number,
  opts: { accent?: number; tape?: boolean } = {},
): void {
  drawPlate(g, x, y, w, h, { fill: Theme.panel, alpha: 0.94 });
  g.fillStyle(Theme.groundDeep, 0.28);
  g.fillRect(x + 4, y + 4, w - 8, h - 8);
  if (opts.tape) {
    drawTapeStrip(g, x + 6, y + 5, Math.min(72, w - 12), 5, opts.accent ?? Theme.tape, 0.75);
  } else if (opts.accent !== undefined) {
    g.fillStyle(opts.accent, 0.85);
    g.fillRect(x + 5, y + 5, 3, h - 10);
  }
  for (const [bx, by] of [
    [x + 4, y + 4],
    [x + w - 5, y + 4],
